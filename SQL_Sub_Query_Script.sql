...
select *
from employee e
where salary > (select avg(salary) from employee e2 where e2.dept_name=e.dept_name) -- this sub-query is dependent on outer query
order by dept_name, salary;
...
-- select 1 from employee e where e.dept_name = 'Finance';
-- select 1 from employee e where e.dept_name = 'Marketing';
...
-- using below condition in order to avoid duplicate records 
-- if employee_history already has some records in it
where not exists (select 1
				  from employee_history eh
				  where eh.emp_id = e.emp_id);

select * from employee_history eh2;
--------------------------------------------------------------------------------
...
/* for MySQL -- You can't specify target table for update in FROM clause
 delete from department d1
where dept_name in(select * from (select dept_name from department d2
				    where not exists (select 1 from employee e
									  where e.dept_name = d2.dept_name)) as t );
 */
